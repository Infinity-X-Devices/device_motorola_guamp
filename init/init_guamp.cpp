    ...
    if (bootsku == "XT2083-3") {
        ...
        if (carrier == "retin") {
            for (const auto &source : RO_PROP_SOURCES) {
                ro_prop_override(source, "device", "guamp", true);
                ro_prop_override(source, "model", "moto g(9)", true);
            }
        } else {
            /* Moto G9 Play */
            for (const auto &source : RO_PROP_SOURCES) {
                ro_prop_override(source, "device", "guamp", true);
                ro_prop_override(source, "model", "moto g(9) play", true);
            }
        }
    } else if (bootsku == "XT2083-4") {
        /* Lenovo K12 Note */
        for (const auto &source : RO_PROP_SOURCES) {
            ro_prop_override(source, "device", "guamp", true);
            ro_prop_override(source, "model", "Lenovo K12 Note", true);
        }
    }
}
